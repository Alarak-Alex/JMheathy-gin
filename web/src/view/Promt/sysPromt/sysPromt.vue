  ...
  <div>
    ...
    <div class="gva-table-box">
        <div class="gva-btn-list">
            ...
            <ExportTemplate v-auth="btnAuth.exportTemplate" template-id="Promt_Promt" />
            <ExportExcel v-auth="btnAuth.exportExcel" template-id="Promt_Promt" />
            <ImportExcel v-auth="btnAuth.importExcel" template-id="Promt_Promt" @on-success="getTableData" />
        </div>
        <el-table
        ref="multipleTable"
        style="width: 100%"
            ...
            <template #default="scope">{{ formatDate(scope.row.CreatedAt) }}</template>
                    ...
                    <el-table-column label="提示词" prop="promtData" width="200">
                        <template #default="scope">
                             <div class="file-list">
                               <el-tag v-for="file in scope.row.promtData" :key="file.uid" @click="downloadFile(file.url)">{{file.name}}</el-tag>
                             </div>
                        </template>
                    </el-table-column>
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button v-auth="btnAuth.edit" type="primary" link icon="edit" class="table-button" @click="updatePromtFunc(scope.row)">变更</el-button>
    ...
    <el-drawer destroy-on-close size="800" v-model="detailShow" :show-close="true" :before-close="closeDetailShow">
            <el-descriptions :column="1" border>
                    <el-descriptions-item label="提示词">
                            <div class="fileBtn" v-for="(item,index) in detailFrom.promtData" :key="index">
...
const getTableData = async() => {
  const table = await getPromtList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deletePromtByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updatePromtFunc = async(row) => {
    const res = await findPromt({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deletePromtFunc = async (row) => {
    const res = await deletePromt({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
